...
# localStorage do navegador. O React (frontend) só chama essas rotas.
import os
import json
import uuid
...
def serialize_return(row, logs, alerts):
    return {
        ...
        'emailsResponsaveis': row['emails_responsaveis'],
...
def fetch_return_full(cur, return_id):
    ...
    alerts = [{
        ...
        'destinatarios': a['destinatarios'], 'assunto': a['assunto'], 'corpoHtml': a['corpo_html']
    ...
    try:
        with conn.cursor() as cur:
            ...
            ''', (
                ...
                payload.get('status', 'Em Trânsito'), psycopg2.extras.Json(payload['emailsResponsaveis'])
            ))
            cur.execute('''
...
def add_email(return_id: str, payload: dict = Body(...)):
    ...
    try:
        with conn.cursor() as cur:
            ...
            if not row:
                raise HTTPException(404, 'Devolução não encontrada')
            emails = row['emails_responsaveis']
            if email in emails:
                raise HTTPException(400, 'Este e-mail já está na lista')
            emails = emails + [email]
            cur.execute('UPDATE returns SET emails_responsaveis = %s WHERE id = %s',
                        (psycopg2.extras.Json(emails), return_id))
            conn.commit()
            return fetch_return_full(cur, return_id)
    finally:
        conn.close()
...
def remove_email(return_id: str, email: str):
    ...
    try:
        with conn.cursor() as cur:
            ...
            if not row:
                raise HTTPException(404, 'Devolução não encontrada')
            emails = [e for e in row['emails_responsaveis'] if e != email]
            cur.execute('UPDATE returns SET emails_responsaveis = %s WHERE id = %s',
                        (psycopg2.extras.Json(emails), return_id))
            conn.commit()
            return fetch_return_full(cur, return_id)
...
def build_email(item, tipo_sla, tipo_alerta):
    ...
    emails_str = ', '.join(escape_html(e) for e in item['emails_responsaveis'])
                    ...
                    if tipo_alerta:
                        ...
                        if not cur.fetchone():
                            ...
                            ''', (new_id('alt'), item['id'], item['cotacao'], now, 'SLA_RESPOSTA', tipo_alerta,
                                  psycopg2.extras.Json(item['emails_responsaveis']), assunto, corpo))
                            send_resend_email(item['emails_responsaveis'], assunto, corpo)
