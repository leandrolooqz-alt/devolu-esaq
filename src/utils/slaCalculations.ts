...
/**
 * Escapa caracteres HTML especiais. Todo campo digitado pelo usuário (cotação,
 * rastreio, motivo/observações) precisa passar por aqui antes de ser
 * interpolado no template do e-mail, já que o corpo é renderizado com
 * dangerouslySetInnerHTML no preview do modal de detalhes. Sem isso, um valor
 * como "<img src=x onerror=...>" executaria no navegador de quem abrir o
 * preview.
 */
...
export function generateSlaEmailAlert(
  ...
  const assunto = `[LOGÍSTICA REVERSA] ${statusStr} - Cotação: ${item.cotacao}`;
  ...
  const corpoHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; background-color: #ffffff;">
      ...
      <p style="font-size: 14px; color: #475569; line-height: 1.5;">
        A devolução registrada para a cotação <strong>${cotacaoSafe}</strong> atingiu o gatilho de notificação para o <strong>${slaNome}</strong>.
      </p>
